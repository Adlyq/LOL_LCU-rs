...
impl ScoutService {
    ...
    pub async fn execute_full_scout(
        ...
        tokio::spawn(async move {
            ...
            let (my_res, their_res) =
                analyze_premade(&api, my_team.clone(), their_team.clone(), 2, 20).await;
            ...
            for (puuid, name) in my_team {
                ...
                set.spawn(async move {
                    ...
                    Some((
                        puuid.clone(),
                        name.clone(),
                        Self::scout_player(&api_c, &puuid, &name).await,
                        false,
                    ))
                });
            }
